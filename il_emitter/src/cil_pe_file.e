...
feature -- Element change
	...
	set_method_writer (a_md_method_writer: MD_METHOD_WRITER)
		...
		do
			...
			md_method_writer_set: md_method_writer = a_md_method_writer
		end
...
feature -- Save
	...
	write_file (a_corFlags: INTEGER)
			...
			--| unset it for standalone assemblies if you want to modify your sdata
		...
		do
			...
			if l_rv and then snk_len /= 0 then
				...
				rsa_encoder.get_strong_name_signature (l_sig_hash, l_sig_len, l_context.message_digest_byte, 20)
					...
					-- TODO review
				output_file.go (snk_base.to_integer_32)
				output_file.put_string (byte_array_to_string (l_sig_hash, l_sig_len.item.to_integer_32))
			end
		end
